...
export default function TestFoodUploadPage() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await fetch(`${BACKEND_URL}/api/content/food/upload`, {
                method: 'POST',
                body: formData,
            });
    ...
    return (
        <div style={{ padding: '2rem', maxWidth: '600px', margin: '0 auto', fontFamily: 'system-ui', color: '#f8fafc' }}>
            <h1 style={{ color: '#ff4d4d', marginBottom: '0.5rem' }}>🍲 Upload Food Video</h1>
            <p style={{ opacity: 0.8, marginBottom: '2rem' }}>Test the <code>/api/content/food/upload</code> endpoint</p>
            ...
            >
                <div style={fieldStyle}>
                    ...
                    <input type="text" name="name" required style={inputStyle} placeholder="Chef Name" />
                </div>

                <div style={fieldStyle}>
                    <label style={labelStyle}>Dish Name:</label>
                    <input type="text" name="dish_name" required style={inputStyle} placeholder="e.g. Traditional Fufu" />
                </div>

                <div style={fieldStyle}>
                    <label style={labelStyle}>Video File (MP4/MOV):</label>
                    <input type="file" name="video" required accept="video/*" style={{ ...inputStyle, padding: '0.4rem' }} />
                </div>
                ...
                <div style={fieldStyle}>
                    ...
                    <textarea name="description" style={{ ...inputStyle, height: '80px' }} placeholder="Tell us about the dish..." />
                </div>

                <div style={fieldStyle}>
                    <label style={labelStyle}>Duration in seconds (Optional):</label>
                    <input type="number" name="duration" style={inputStyle} placeholder="e.g. 60" />
                </div>
                ...
                >
                    {loading ? 'Uploading...' : 'Upload Food Video'}
                </button>
            </form>
