...
const CreatorWizard = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    documents: {
      id_url: null,
      id_name: null,
      endorsements_url: null,
      endorsements_name: null,
    },
  });
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      if (!formData.category)
        newErrors.category = "Please select a creator category to continue.";
    } else if (step === 2) {
      ...
      if (!id.ethnicGroup)
        newErrors.ethnicGroup = "Ethnic Group / Community is required.";
      if (!id.phone) newErrors.phone = "Phone number is required.";
      if (!id.email) newErrors.email = "Email is required.";
      else if (!/\S+@\S+\.\S+/.test(id.email))
        newErrors.email = "Please enter a valid email address.";
    } else if (step === 3) {
      const pro = formData.professional || {};
      if (!pro.languages) newErrors.languages = "Languages is required.";
      if (
        pro.experienceTime === undefined ||
        pro.experienceTime === null ||
        pro.experienceTime === ""
      ) {
        newErrors.experienceTime = "Experience is required.";
      }
      if (!pro.bio || pro.bio.length < 10)
        newErrors.bio = "Bio must be at least 10 characters.";
    } else if (step === 4) {
      if (!formData.documents?.id_url)
        newErrors.id = "Please upload your Government ID to continue.";
  ...
  return (
    <div
      ...
      style={{ fontFamily: "'Inter', sans-serif", backgroundColor: "#0B0E14" }}
    >
      <nav
        style={{
        ...
        className="h-14 px-8 flex items-center justify-between flex-shrink-0 z-30"
      >
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="hidden lg:block w-[40%] flex-shrink-0 relative overflow-hidden">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(to bottom, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.4) 60%, rgba(0,0,0,0.3) 100%)",
            }}
          />
          <div
            className="absolute inset-0"
          ...
          <div className="absolute inset-0 flex flex-col justify-start p-10 pt-12">
            <div className="space-y-4">
              ...
              <p className="text-gray-200 text-sm font-semibold leading-relaxed">
                Join SawaFlix as a verified creator and preserve your cultural
                heritage for future generations while earning from your
                authentic content.
              </p>
            </div>
          </div>
        ...
        <div className="flex-1" style={{ backgroundColor: "#0B0E14" }}>
          <div
            className="h-full overflow-y-auto px-6 lg:px-10 py-5"
                      ...
                      <Step3Professional
                        ...
                        onChange={(e) =>
                          updateFormData("professional", {
                            experienceTime: Number(e.target.value),
                          })
                        }
                      />
                    )}
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-[10px] leading-tight mt-0.5">
                        Join our growing community of verified cultural creators
                      </p>
                    </div>
                  </div>
                  <div
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-[10px] leading-tight mt-0.5">
                        Curated platform ensuring authentic cultural content
                      </p>
                    </div>
                  </div>
                </div>
