...
const Step5Summary = ({ formData, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    await onSubmit();
    setSubmitted(true);
    // Optionally, you can also scroll to top or redirect here
  };
  ...
  const SummaryItem = ({ label, value }) => (
    <div className="space-y-0.5">
      ...
      <p className="text-gray-300 font-bold text-xs truncate">
        {value || "Not provided"}
      </p>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      {submitted && (
        <div className="w-full bg-green-600/90 text-white text-center font-bold py-3 rounded-xl shadow-lg mb-2 animate-in fade-in duration-300">
          🎉 Application submitted successfully! Your application is now under
          review.
        </div>
      )}
      <div>
        <h2 className="text-xl font-bold text-white tracking-tight">
          Review & <span className="text-red-600">Submit</span>
        </h2>
        ...
        <div className="bg-[#1A1F2B] border border-white/5 rounded-xl p-4 shadow-xl shadow-black/20">
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            {(portfolio.recordings || []).map((rec, i) => (
              ...
              >
                <p className="text-[8px] font-black text-red-600 mb-0.5 uppercase">
                  Entry {i + 1}
                </p>
                <p className="font-bold text-gray-300 text-[9px] truncate">
                  {rec.title || "Untitled"}
                </p>
                {rec.file_name && (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center gap-2 bg-black/20 p-2 rounded-lg border border-white/5">
              ...
              <span className="text-gray-300 font-bold text-[9px] truncate">
                {documents.id_name || "ID Uploaded"}
              </span>
            </div>
            <div className="flex items-center gap-2 bg-black/20 p-2 rounded-lg border border-white/5">
              ...
              <span className="text-gray-300 font-bold text-[9px] truncate">
                {documents.endorsements_name || "No Endorsements"}
              </span>
            </div>
          </div>
        </div>
      ...
      <form
        className="flex flex-col items-center pt-2 space-y-4"
      ...
      >
        <label className="flex items-center gap-3 cursor-pointer group max-w-sm text-center">
          <input
            type="checkbox"
            required
            className="w-4 h-4 bg-black border-white/10 rounded checked:bg-red-600 transition-all cursor-pointer"
          />
          <span className="text-gray-500 text-[10px] font-medium leading-tight">
            I confirm information accuracy and agree to Terms of Service.
          </span>
        </label>
