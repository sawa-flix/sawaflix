...
// ─── Search Result Card (TikTok portrait, mostly white) ───────────────────────
const SearchResultCard = ({ video, onPlay }) => (
  ...
  >
    ...
    {/* Centre play button — white circle, red icon */}
...
const VideoFeedItem = ({ video, isActive, isMuted, setIsMuted }) => {
  ...
  // ── Tap to play/pause OR double-tap to seek 10 s ─────────────────────────
  ...
  return (
    <div className="relative w-full h-[85vh] bg-black rounded-3xl overflow-hidden mb-6 shadow-2xl border border-white/5 group/vid">
      ...
      <button
        ...
        className="absolute top-5 right-5 p-2.5 bg-black/40 backdrop-blur-md rounded-full text-white border border-white/10 z-30"
      >
      ...
      <div className="absolute inset-x-0 bottom-0 z-30 flex flex-col pointer-events-none">
        ...
        <div
          ...
          className="w-full px-4 pb-4 pointer-events-auto"
        >
          ...
          >
            ...
            {/* Thumb — visible on hover or when dragging */}
            <div
              className={`absolute w-3.5 h-3.5 bg-white rounded-full shadow-lg -translate-x-1/2 transition-opacity ${isDragging ? 'opacity-100 scale-125' : 'opacity-0 group-hover/scrub:opacity-100'}`}
              style={{ left: `${progress}%` }}
            />
          </div>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            >
              {isPaused
                ...
                : <div className="flex gap-0.5"><div className="w-1.5 h-4 bg-white rounded-sm"/><div className="w-1.5 h-4 bg-white rounded-sm"/></div>
              }
            </button>
      ...
      {commentOpen && (
        <div className="absolute inset-0 z-40 flex flex-col justify-end">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setCommentOpen(false)} />
          <div className="relative bg-[#13161f] rounded-t-[2.5rem] p-6 h-[70%] flex flex-col border-t border-white/5">
...
function SawaFlixContent() {
  ...
  // When a search card is clicked, we store that video to display first in feed-mode
  ...
  // ── Scroll to discover when header search fires (full scroll past header) ──
  useEffect(() => {
    if (urlQuery) {
      const t = setTimeout(() => {
  ...
  // ── Reset selectedVideo when search changes ──
  ...
  // Precise scroll that accounts for the fixed 64px header
  ...
  // In card-click feed mode: selected video first, then the rest
  const feedVideos = (() => {
    ...
    <div className="min-h-screen bg-[#0B0E14] flex flex-col overflow-x-hidden">
      <main className="flex-1 p-2 sm:p-6 lg:p-8 pt-0 sm:pt-6">
        ...
        <section className="relative h-[85vh] sm:h-[75vh] lg:h-[80vh] rounded-[2.5rem] overflow-hidden mb-10 group shadow-2xl border border-white/5 bg-black">
          ...
          {/* Centre: white circle + red play icon + Play Now label */}
        ...
        <section ref={discoverRef} className="mb-12">
          <div className="flex items-center justify-between mb-8 gap-4">
            ...
            {/* Back-to-cards button (visible when in feed-from-card mode) */}
            {isSearchMode && selectedVideo && (
              <button
          ...
          {loading ? (
            ...
            /* ── Search result cards (portrait grid) ── */
            ...
            /* ── TikTok vertical feed (normal or after card click) ── */
