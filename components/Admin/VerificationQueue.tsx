      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "INSERT") {
            ...
            setItems((prev) =>
              prev.filter(
                (item) => item.id !== (payload.old as VerificationItem).id,
              ),
            );
          }
        },
        ...
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-white">
              No creators available for review
            </h3>
            <p className="text-gray-400 mt-2 max-w-sm">
              There are no pending verification requests at the moment.
              Submissions will appear here automatically when creators apply.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
                  ...
                  filteredItems.map((item) => {
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-gray-800 overflow-hidden border border-gray-700 shrink-0">
                              <img
                                src={
                                  item.avatar_url ||
                                  `https://ui-avatars.com/api/?name=${encodeURIComponent(item.full_name)}&background=333&color=fff`
                                }
                                alt={item.full_name}
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${st.className}`}
                          >
                            {st.icon}
                            {st.label}
                          </span>
