...
function Toast({
  ...
  return (
    <div
      className={`fixed top-6 right-6 z-[100] flex items-center gap-3 px-5 py-4 rounded-xl shadow-2xl border transition-all animate-in slide-in-from-top duration-300 ${type === "success" ? "bg-green-500/10 border-green-500/30 text-green-400" : "bg-red-500/10 border-red-500/30 text-red-400"}`}
    >
      {type === "success" ? (
        <CheckCircle size={20} />
      ) : (
...
export default function VerificationDetails({ id }: { id: string }) {
  ...
  const handleAction = async (type: "approve" | "reject" | "info") => {
    ...
    const endpointMap: Record<string, string> = {
      ...
      info: `/api/admin/verifications/${id}/info-request`,
    };
    ...
    try {
        ...
        title:
          type === "approve"
            ...
            : type === "reject"
              ? "Creator Rejected"
              : "Info Requested",
        message: `${creatorName} has been ${
          type === "approve"
            ? "approved"
            : type === "reject"
              ? "rejected"
              : "sent a request for more info"
        }.`,
      });
      ...
      if (type === "approve") {
        setToast({
          message: "Creator approved successfully! Redirecting...",
          type: "success",
        });
        ...
        setToast({
          message: "Submission rejected. Redirecting...",
          type: "success",
        });
        ...
        setToast({
          message: "Message sent to creator. Submission stays in queue.",
          type: "success",
        });
  ...
  const isActionable = true; // Always allow actions
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-24">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="ml-auto">
          <span
            className={`px-3 py-1 rounded-full text-sm font-medium border ${currentStatus.className}`}
          >
            {currentStatus.label}
          </span>
        </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {data.professional.genre.map((g) => (
                      <span
                        key={g}
                        className="px-2 py-0.5 rounded bg-gray-800 text-gray-300 text-xs border border-gray-700"
                      >
                        {g}
                      </span>
                    ))}
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              {Object.entries(data.documents).map(([key, url]) => {
                if (key === "idCardUrl" || key === "selfieUrl" || !url)
                  return null;
                return (
      ...
      <div className="fixed bottom-0 left-0 right-0 lg:left-64 bg-gray-900/95 backdrop-blur border-t border-gray-800 p-4 z-40">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-3">
          <p className="text-xs text-gray-500 hidden sm:block">
            {!isActionable
              ? `This submission is already ${data.status.replace("_", " ")}.`
